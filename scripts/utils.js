...
// This code was originally an open source clock, which I have modified to suit the needs of the status message.
function startTime()
{
	var today=new Date();
	...
	m=checkTime(m);
	s=checkTime(s);
	document.getElementById('pstatusTime').innerHTML=today.toDateString() + " " + today.toTimeString().replace(/.*(\d{2}:\d{2})(:\d{2}).*/, "$1");
